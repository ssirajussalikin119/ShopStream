...
const PaymentModal = ({ isOpen, onClose, product }) => {
  ...
  const handlePay = async () => {
    ...
    try {
      const response = await paymentAPI.initiate(product._id, product.price, {
        name: form.name.trim(),
  ...
  return (
    <>
      ...
      <aside
        className={`fixed right-0 top-0 z-[310] h-full w-full max-w-[460px] bg-white shadow-2xl flex flex-col transition-transform duration-300 ${
