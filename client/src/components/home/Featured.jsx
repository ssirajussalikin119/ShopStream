...
const Featured = () => {
  ...
  useEffect(() => {
    ...
    productAPI.getFeatured()
      .then((data) => { if (isMounted) { setProducts(data); setError(""); } })
      .catch((e) => { if (isMounted) setError(e?.message || "Unable to load featured products."); })
    ...
    <section className="bg-gray-50 py-20">
      <Container className="mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between mb-10">
          ...
          <Link to="/category/electronics" className="text-blue-600 font-semibold hover:text-blue-700 transition text-sm">
            View All →
          </Link>
        </div>
        ...
        {loading && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {[1,2,3,4].map((i) => (
              <div key={i} className="bg-white p-4 rounded-md shadow-sm border border-gray-100 animate-pulse">
        ...
        {!loading && !error && products.length === 0 && (
          <div className="rounded-md border border-gray-200 bg-white px-4 py-6 text-sm text-gray-600">No featured products yet.</div>
        )}

        {!loading && !error && products.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {products.map((product) => (
              <div key={product._id} className="bg-white p-4 rounded-md shadow-sm border border-gray-100 group transition-all hover:shadow-md">
                ...
                <Link to={`/product/${product._id}`}>
                  <h3 className="font-bold text-gray-900 mb-2 truncate hover:text-blue-600 transition-colors">{product.name}</h3>
                </Link>
