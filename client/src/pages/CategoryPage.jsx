...
const CategoryPage = () => {
  ...
  const toggleFavourite = (e, id) => {
    e.preventDefault();
    e.stopPropagation();
    const updated = favourites.includes(id) ? favourites.filter((x) => x !== id) : [...favourites, id];
  ...
  const activeFilterCount =
    selectedBrands.length + selectedSubcategories.length + (selectedPriceRange !== 'all' ? 1 : 0);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {activeFilterCount > 0 && (
            <span className="bg-blue-600 text-white text-xs font-bold rounded-full px-2 py-0.5">{activeFilterCount}</span>
          )}
        </div>
        {activeFilterCount > 0 && (
          <button
            className="text-sm font-semibold text-blue-600 hover:text-blue-700"
            onClick={() => { setSelectedBrands([]); setSelectedSubcategories([]); setSelectedPriceRange('all'); setSortBy('featured'); }}
          >
            Reset all
          </button>
        )}
      ...
      <div className="border-t border-gray-100 pt-5">
        <h3 className="mb-3 text-sm font-bold uppercase tracking-widest text-gray-700">Price</h3>
        <div className="space-y-2.5">
          {PRICE_OPTIONS.map((option) => (
            <label key={option.value} className="flex items-center gap-3 text-sm text-gray-700 cursor-pointer group">
              <input type="radio" name="price-range" checked={selectedPriceRange === option.value} onChange={() => setSelectedPriceRange(option.value)} className="accent-blue-600" />
      ...
      {filters.brands.length > 0 && (
        <div className="border-t border-gray-100 pt-5">
          <h3 className="mb-3 text-sm font-bold uppercase tracking-widest text-gray-700">Brand</h3>
          <div className="space-y-2.5">
            {filters.brands.map((brand) => (
              <label key={brand} className="flex items-center gap-3 text-sm text-gray-700 cursor-pointer group">
                <input type="checkbox" checked={selectedBrands.includes(brand)} onChange={() => toggleSelection(brand, selectedBrands, setSelectedBrands)} className="accent-blue-600" />
      ...
      {filters.subcategories.length > 0 && (
        <div className="border-t border-gray-100 pt-5">
          <h3 className="mb-3 text-sm font-bold uppercase tracking-widest text-gray-700">Product Type</h3>
          <div className="space-y-2.5">
            {filters.subcategories.map((subcategory) => (
              <label key={subcategory} className="flex items-center gap-3 text-sm text-gray-700 cursor-pointer group">
                <input type="checkbox" checked={selectedSubcategories.includes(subcategory)} onChange={() => toggleSelection(subcategory, selectedSubcategories, setSelectedSubcategories)} className="accent-blue-600" />
                <span className="group-hover:text-blue-600 transition-colors">{subcategory}</span>
              </label>
            ))}
          </div>
        </div>
      ...
      <Container className="mx-auto px-4 sm:px-6">
        ...
        <div className="relative rounded-3xl overflow-hidden mb-8 h-40 sm:h-52">
          ...
          <div className="absolute inset-0 flex items-center px-8">
            <div>
              ...
              <p className="text-white/80 text-sm mt-1 max-w-xs">{category.description}</p>
            </div>
          </div>
        </div>
        ...
        <section className="grid gap-8 lg:grid-cols-[260px_1fr]">
          ...
          <div>
            ...
            <div className="mb-6 flex flex-col gap-3 rounded-3xl border border-gray-100 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-3">
                ...
                >
                  ...
                  Filters {activeFilterCount > 0 && <span className="bg-blue-600 text-white text-xs rounded-full px-1.5 py-0.5">{activeFilterCount}</span>}
                </button>
                <div>
              ...
              <label className="flex items-center gap-2 text-sm font-semibold text-gray-700 self-start sm:self-center">
                ...
                <div className="relative">
                  <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="appearance-none rounded-full border border-gray-200 bg-gray-50 py-2 pl-4 pr-10 text-sm outline-none focus:border-blue-400">
            ...
            {error && (
              <div className="rounded-3xl border border-red-100 bg-red-50 px-5 py-4 text-sm text-red-700 mb-6">{error}</div>
            )}
            ...
            {!loading && !error && products.length === 0 && (
              <div className="rounded-3xl border border-gray-100 bg-white px-6 py-16 text-center shadow-sm">
                <SlidersHorizontal size={40} className="mx-auto mb-4 text-gray-200" />
                <h3 className="text-xl font-bold text-gray-900 mb-2">No products match these filters</h3>
                <p className="text-gray-500 mb-5">Try changing the brand, price range, or product type.</p>
                <button
                  onClick={() => { setSelectedBrands([]); setSelectedSubcategories([]); setSelectedPriceRange('all'); }}
                  ...
                  return (
                    ...
                    >
                      <div className="relative aspect-[4/3] overflow-hidden bg-gray-100">
                        ...
                        >
                          <Heart size={16} className={favourites.includes(product._id) ? 'fill-red-500 text-red-500' : 'text-gray-400'} />
                        </button>
                      </div>
                      ...
                      <div className="p-5">
                        ...
                        <p className="mb-3 text-sm leading-6 text-gray-500 line-clamp-2">{product.description}</p>
                        ...
                        <div className="mb-4 flex items-end gap-2">
                          ...
                          {product.compareAtPrice > product.price && (
                            <span className="text-sm font-medium text-gray-400 line-through mb-0.5">{formatPrice(product.compareAtPrice)}</span>
                          )}
                        </div>
