...
const updateItem = asyncHandler(async (req, res) => {
  ...
  if (Number(quantity) === 0) {
    ...
    // Stock check
    const product = await Product.findById(productId).select("stockCount").lean();
...
const checkout = asyncHandler(async (req, res) => {
  ...
  const orderSummary = {
    ...
    tax: +(cart.subtotal * 0.08).toFixed(2),
    total: +(cart.subtotal * 1.08).toFixed(2),
  ...
  return sendResponse(res, 200, true, "Order placed successfully", orderSummary);
});
